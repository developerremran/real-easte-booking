import { AiOutlineMenu } from 'react-icons/ai'
import { useState } from 'react'
import { Link } from 'react-router-dom'


import profile from '../../Image/placeholder.jpg'
import ContextShear from '../../API/ContextShear'
import MainModal from '../../COmponents/Modal/MainModal'
import { setHost } from '../../API/auth'
import { toast } from 'react-hot-toast'

const MenuDropdown = () => {

  const { user, logOut,setRole ,role} = ContextShear();

  const [isOpen, setIsOpen] = useState(false)
  const [modal, setModal] = useState(false)
  //   const toggleOpen = useCallback(() => {
  //     setIsOpen(value => !value)
  //   }, [])
  const logOutUser = () => {
    logOut()
  }
  const modalHandler = email => {
    setHost(email)
    .then(data => {
      console.log(data)
      toast.success('You are host now, Post Rooms!')
      setRole('host')
      closeModal()
    })
  }

  const closeModal = () => {
    setModal(false)
  }
    // console.log(user);
  return (
    <div className='relative'>
      <div className='flex flex-row items-center gap-3 py-3 px-8'>
        {/* Aircnc btn */}

     {
      user?
      <>
      <div className='hidden md:block text-sm font-semibold rounded-full py-3 px-8   transition'>
      <button
       className='cursor-pointer hover:bg-neutral-100 py-3 px-4 '
       onClick={() => setModal(true)}
       disabled={!user}
     >
       AirCNC your home
     </button>
   </div>
      </>
      :
      <>
      </>
     }
        {/* Dropdown btn */}
        <div
          onClick={() => setIsOpen(!isOpen)}
          className='p-4 md:py-1 md:px-2 border-[1px] border-neutral-200 flex flex-row items-center gap-3 rounded-full cursor-pointer hover:shadow-md transition'
        >
          <AiOutlineMenu />
          <div className='hidden md:block'>
            {
              user ?
                <>
                  <img className='w-[ 30px] h-[30px] rounded-full' src={user.photoURL} alt="" />

                </>
                :
                <>
                  <img className='w-[ 30px] h-[30px] rounded-full' src={profile} alt="" />
                </>
            }


          </div>
        </div>
      </div>
      {isOpen && (
        <div className='absolute rounded-xl shadow-md w-[40vw] md:w-3/4 bg-white overflow-hidden right-0 top-12 text-sm'>
          <div className='flex flex-col cursor-pointer'>
            <Link
              to='/'
              className='block md:hidden px-4 py-3 hover:bg-neutral-100 transition font-semibold'
            >
              Home
            </Link>
            {user ? (
              <>
              <Link to='admin-dashboard'
                
                className='px-4 py-3 hover:bg-neutral-100 transition font-semibold cursor-pointer'
              >
                Dashboard
              </Link>
              <div
                onClick={logOutUser}
                className='px-4 py-3 hover:bg-neutral-100 transition font-semibold cursor-pointer'
              >
                Logout
              </div>
              </>
            ) : (
              <>
                <Link
                  to='/login'
                  className='px-4 py-3 hover:bg-neutral-100 transition font-semibold'
                >
                  Login
                </Link>
                <Link
                  to='/register'
                  className='px-4 py-3 hover:bg-neutral-100 transition font-semibold'
                >
                  Sign Up
                </Link>
              </>
            )}
          </div>
        
        </div>
      )}
      <MainModal
      email={user?.email}
      modalHandler={modalHandler}
      isOpen={modal}
      closeModal={closeModal}
      >

      </MainModal>
    </div>
  )
}

export default MenuDropdown
